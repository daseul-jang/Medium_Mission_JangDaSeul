...
export default function LatestListSm() {
  ...
  const handlePrevClick = (e: React.MouseEvent) => {
    ...
    setCurrentIndex((prevIndex) =>
      prevIndex > 0 ? prevIndex - 1 : Math.ceil(posts.length / 2) - 1
    );
  };
  ...
  const handleNextClick = (e: React.MouseEvent) => {
    ...
    // 다음 버튼 클릭 핸들러
    setCurrentIndex((prevIndex) =>
      prevIndex < Math.ceil(posts.length / 2) - 1 ? prevIndex + 1 : 0
    );
  };

  const currentPosts = posts?.slice(currentIndex * 2, (currentIndex + 1) * 2);
    ...
    <div className='w-full relative'>
      ...
      <ul className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 py-4 px-8'>
        {currentPosts?.map((post: Post, index: number) => (
          <Link href={`/posts/${post.id}`} key={post.id} className='h-36'>
            <li className='card card-side bg-base-100 rounded-sm w-full h-full p-3 shadow-md'>
              <figure className='basis-1/6'>
                <span className='w-full h-full flex font-bold text-3xl text-base-300'>
                  {(currentIndex * 2 + index + 1).toString().padStart(2, '0')}
                </span>
              </figure>
              <div className='card-body p-2 basis-5/6 h-full gap-0'>
                <span className='card-actions justify-start text-xs'>
                  {post.writer.username}
                </span>
                <h3 className='card-title text-md h-full max-h-14 overflow-ellipsis overflow-hidden line-clamp-2'>
                  {post.title}
                </h3>
